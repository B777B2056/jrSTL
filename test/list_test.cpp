...
TEST(testCase,list_iterator_ctor_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    jrSTL::list<int> src(cnt, var);
    // 迭代器范围构造函数
    jrSTL::list<int> des(src.begin(), src.end());
...
TEST(testCase,list_move_ctor_test) {
    ...
    jrSTL::list<int> src(cnt, var);
    // 移动构造函数
    jrSTL::list<int> des(std::move(src));
...
TEST(testCase,list_initializer_list_ctor_test) {
    ...
    jrSTL::list<int> des{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    ASSERT_EQ(src.size(), des.size());
    auto it = src.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
...
TEST(testCase, list_swap_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    jrSTL::list<int> src(cnt, var - 1);
    jrSTL::list<int> des(cnt, var + 1);
    jrSTL::list<int> tmp = src;
    src.swap(des);
    ASSERT_EQ(tmp.size(), des.size());
    auto it = tmp.begin();
    auto dit = des.begin();
...
TEST(testCase, list_merge_test) {
    ...
    jrSTL::list<int> des0{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    jrSTL::list<int> des1{33,53421,4247,20,234,25,48,20,2350,18,2423,601,474,67,5589,2089};
    // 同长度
    src0.sort();
    src1.sort();
    des0.sort();
...
TEST(testCase, list_splice_test) {
    ...
    jrSTL::list<int> des2{33,53421,4247,20,234,25,48};
    ...
    jrSTL::list<int> des3{33,53421,4247,20,234,25,48};
    auto i0 = src0.begin();
    auto j0 = des0.cbegin();
    for(int i = 0; i < 4; i++) {
        ++i0;
...
TEST(testCase, list_unique_test) {
    ...
    jrSTL::list<int> des0{32,53423,4245,25,234,25,45,2,4245,25,234,25,45,2,4245,25,234,25,45,2,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    src0.unique();
    des0.unique();
    ASSERT_EQ(src0.size(), des0.size());
    auto it0 = src0.begin();
