...
TEST(testCase, shared_ptr_ctor_test)
{
    ...
    {
        jr_std::shared_ptr<Foo_j> sh1_j;
    }

    {
        jr_std::shared_ptr<Foo_j> sh2_j(new Foo_j);
        jr_std::shared_ptr<Foo_j> sh3_j(sh2_j);
        ctor0 += std::to_string(sh2_j.use_count());
        ctor0 += std::to_string(sh3_j.use_count());
    }

    {
        jr_std::shared_ptr<Foo_j> sh4_j(new Foo_j, D_j());
        jr_std::shared_ptr<Foo_j> sh5_j(new Foo_j, [](Foo_j *p) {
           delete p;
        });
    }
...
TEST(testCase, shared_ptr_reset_test)
{
    ...
    jr_std::shared_ptr<Foo_j> sptr_j = jr_std::make_shared<Foo_j>(1);
...
void fun(jr_std::shared_ptr<int> sp)
{
...
TEST(testCase, shared_ptr_use_count_test)
{
    ...
    auto sp2 = jr_std::make_shared<int>(5);
...
TEST(testCase, shared_ptr_type_cast_test) {
    ...
    jr_std::shared_ptr<BaseClass_j> ptr_to_base0(jr_std::make_shared<DerivedClass_j>());

    jr_std::static_pointer_cast<DerivedClass_j>(ptr_to_base0)->f(); // OK
    // （构造临时 shared_ptr ，然后调用 operator-> ）
