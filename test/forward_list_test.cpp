...
TEST(testCase,forward_list_assign_ctor_test){
    ...
    // 赋值构造函数测试
    panzer::forward_list<int> des(cnt, var);
    // 迭代器遍历测试
    auto dit = des.begin();
    for(; dit != des.end(); ++dit)
        EXPECT_EQ(var, *dit);
...
TEST(testCase,forward_list_iterator_ctor_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::forward_list<int> src(cnt, var);
    // 迭代器范围构造函数
    panzer::forward_list<int> des(src.begin(), src.end());
    auto it = src.begin();
    auto dit = des.begin();
...
TEST(testCase,forward_list_copy_ctor_test) {
    ...
    panzer::forward_list<int> src(cnt, var);
    // 拷贝构造函数
    panzer::forward_list<int> des(src);
    auto it = src.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
...
TEST(testCase,forward_list_move_ctor_test) {
    ...
    panzer::forward_list<int> src(cnt, var);
    // 移动构造函数
    panzer::forward_list<int> des(std::move(src));
...
TEST(testCase,forward_list_initializer_list_ctor_test) {
    ...
    panzer::forward_list<int> des{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    auto it = src.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
...
TEST(testCase, forward_list_assign_mem_fn_test) {
    ...
    panzer::forward_list<int> des(15, -4);
    // 重复赋值测试
    src.assign(cnt, var);
    des.assign(cnt, var);
    auto it = src.begin();
    ...
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
    // 迭代器赋值测试
    panzer::forward_list<int> tmp(6, -100);
...
TEST(testCase, forward_list_resize_test) {
    ...
    // 新size小于旧size
    des.resize(cnt / 2);
    ASSERT_EQ(panzer::distance(des.begin(), des.end()), cnt / 2);
    auto it = tmp.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
    // 新size大于旧size
    des.resize(cnt, var);
    ASSERT_EQ(panzer::distance(des.begin(), des.end()), cnt);
    it = tmp.begin();
    dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
...
TEST(testCase, forward_list_push_front_test) {
    ...
    panzer::forward_list<int> des;
    // 拷贝push
    for(int n : {32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2}) {
        src.push_front(n);
        des.push_front(n);
    }
    ASSERT_EQ(std::distance(src.begin(), src.end()), panzer::distance(des.begin(), des.end()));
    auto it = src.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
...
TEST(testCase, forward_list_emplace_front_test) {
    ...
    EXPECT_EQ(std::distance(src.begin(), src.end()), panzer::distance(des.begin(), des.end()));
    auto it = src.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
...
TEST(testCase, forward_list_insert_test) {
    ...
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
    // 迭代器insert
    panzer::forward_list<int> tmp({1,2,3,4,5});
...
TEST(testCase, forward_list_swap_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::forward_list<int> src(cnt, var - 1);
    panzer::forward_list<int> des(cnt, var + 1);
    panzer::forward_list<int> tmp = src;
    src.swap(des);
    ASSERT_EQ(panzer::distance(src.begin(), src.end()), panzer::distance(des.begin(), des.end()));
    auto it = tmp.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
        EXPECT_EQ(*it, *dit);
...
TEST(testCase, forward_list_merge_test) {
    ...
    panzer::forward_list<int> des0{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    panzer::forward_list<int> des1{33,53421,4247,20,234,25,48,20,2350,18,2423,601,474,67,5589,2089};
    // 同长度
    src0.sort();
    src1.sort();
    des0.sort();
    ...
    src0.merge(src1);
    des0.merge(des1);
    ASSERT_EQ(std::distance(src0.before_begin(), src0.end()), panzer::distance(des0.before_begin(), des0.end()));
    ASSERT_EQ(std::distance(src1.before_begin(), src1.end()), panzer::distance(des1.before_begin(), des1.end()));
    auto it0 = src0.begin();
...
TEST(testCase, forward_list_splice_test) {
    ...
    ASSERT_EQ(std::distance(src0.begin(), src0.end()), panzer::distance(des0.begin(), des0.end()));
    auto it0 = src0.begin();
    auto dit0 = des0.begin();
    for(; dit0 != des0.end(); ++dit0, ++it0)
        EXPECT_EQ(*it0, *dit0);
    ...
    panzer::forward_list<int> des3{33,53421,4247,20,234,25,48};
    auto i0 = src0.begin();
    auto j0 = des0.cbegin();
    for(int i = 0; i < 4; i++) {
        ++i0;
...
TEST(testCase, forward_list_unique_test) {
    ...
    panzer::forward_list<int> des0{32,53423,4245,25,234,25,45,2,4245,25,234,25,45,2,4245,25,234,25,45,2,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    src0.unique();
    des0.unique();
    ASSERT_EQ(std::distance(src0.begin(), src0.end()), panzer::distance(des0.begin(), des0.end()));
    auto it0 = src0.begin();
