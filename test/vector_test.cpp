...
// 赋值构造函数与元素访问测试
TEST(testCase,vector_assign_ctor_test){
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    // 赋值构造函数测试
    std::vector<int> src(cnt, var);
    panzer::vector<int> des(cnt, var);
...
// 迭代器范围构造函数测试
TEST(testCase,vector_iterator_ctor_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> src(cnt, var);
    // 迭代器范围构造函数
    panzer::vector<int> des(src.begin(), src.end());
    ASSERT_EQ(src.size(), des.size());
    for(size_t i = 0; i < src.size(); i++)
        EXPECT_EQ(src[i], des[i]);
}
...
// 拷贝构造函数测试
TEST(testCase,vector_copy_ctor_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> src(cnt, var);
    // 拷贝构造函数
    panzer::vector<int> des(src);
    ASSERT_EQ(src.size(), des.size());
    for(size_t i = 0; i < src.size(); i++)
        EXPECT_EQ(src[i], des[i]);
}
...
// 移动构造函数测试
TEST(testCase,vector_move_ctor_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> src(cnt, var);
    // 移动构造函数
    panzer::vector<int> des(std::move(src));
    ASSERT_EQ(src.empty(), true);
}

// 初始化列表构造函数测试
TEST(testCase,vector_initializer_list_ctor_test) {
    std::vector<int> src{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    // 迭代器范围构造函数
    panzer::vector<int> des{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    ASSERT_EQ(src.size(), des.size());
    for(size_t i = 0; i < src.size(); i++)
        EXPECT_EQ(src[i], des[i]);
}
...
// 拷贝运算符测试
TEST(testCase, vector_operator_copy_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> src(cnt, var);
...
// 移动运算符测试
TEST(testCase, vector_operator_move_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> src(cnt, var);
...
// 赋值成员函数测试
TEST(testCase, vector_assign_mem_fn_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    std::vector<int> src(10, -1);
    panzer::vector<int> des(15, -4);
    // 重复赋值测试
    src.assign(cnt, var);
    des.assign(cnt, var);
    for(size_t i = 0; i < des.size(); ++i)
...
// clear测试
TEST(testCase, vector_clear_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> des(cnt, var);
...
// resize测试
TEST(testCase, vector_resize_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> des(cnt, var);
...
// push_back测试
TEST(testCase, vector_push_back_test) {
    std::vector<int> src;
    panzer::vector<int> des;
    // 拷贝push
...
// pop_back测试
TEST(testCase, vector_pop_back_test) {
    std::vector<int> src{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    panzer::vector<int> des{32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2};
    src.pop_back();
    des.pop_back();
...
// 原位构造push测试
TEST(testCase, vector_emplace_back_test) {
    std::vector<int> src;
    panzer::vector<int> des;
    for(int n : {32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2}) {
        src.emplace_back(n);
...
// insert测试
TEST(testCase, vector_insert_test) {
    std::vector<int> src;
    panzer::vector<int> des;
    // 拷贝insert
    for(int n : {32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2}) {
...
// emplace测试
TEST(testCase, vector_emplace_test) {
    std::vector<int> src;
    panzer::vector<int> des;
    // 拷贝emplace
    for(int n : {32,53423,4245,25,234,25,45,2,235,23,24,6,47,6,5224,2}) {
...
// erase测试
TEST(testCase, vector_erase_test) {
    size_t cnt;
    int var;
...
// swap测试
TEST(testCase, vector_swap_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    panzer::vector<int> src(cnt, var - 1);
    panzer::vector<int> des(cnt, var + 1);
    panzer::vector<int> tmp = src;
    src.swap(des);
    ASSERT_EQ(tmp.size(), des.size());
...
TEST(testCase, vector_shrink_to_fit_test) {
    panzer::vector<int> des(5, 1);
    des.reserve(100);
...
// 逆序访问测试
TEST(testCase, vector_reverse_test) {
    size_t cnt;
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    std::vector<unsigned int> src;
    panzer::vector<unsigned int> des;
