...
// 消费 unique_ptr 的函数能以值或以右值引用接收它
jr_std::unique_ptr<D_j> pass_through(jr_std::unique_ptr<D_j> p)
{
    p->bar();
    return p;
}
...
void jr_std_unique() {
    {
        auto p = jr_std::unique_ptr<D_j>(new D_j()); // p 是占有 D 的 unique_ptr
        auto q = pass_through(std::move(p));
        assert(!p); // 现在 p 不占有任何内容并保有空指针
        q->bar();   // 而 q 占有 D 对象
    } // ~D 调用于此
    {
      jr_std::unique_ptr<B_j> p = jr_std::unique_ptr<D_j>(new D_j()); // p 是占有 D 的 unique_ptr
                                                    // 作为指向基类的指针
      p->bar(); // 虚派发

      std::vector<jr_std::unique_ptr<B_j>> v;  // unique_ptr 能存储于容器
      v.push_back(jr_std::unique_ptr<D_j>(new D_j()));
      v.push_back(std::move(p));
    ...
    std::ofstream("demo.txt") << 'x'; // 准备要读的文件
    {
        jr_std::unique_ptr<std::FILE, void (*)(std::FILE*) > fp(std::fopen("demo.txt", "r"),
                                                             close_file);
        if(fp) // fopen 可以打开失败；该情况下 fp 保有空指针
          (char)std::fgetc(fp.get());
    } // fclose() 调用于此，但仅若 FILE* 不是空指针
    ...
    {
      jr_std::unique_ptr<D_j, std::function<void(D_j*)>> p(new D_j, [](D_j* ptr)
          {
              delete ptr;
          });  // p 占有 D
      p->bar();
    ...
    {
        jr_std::unique_ptr<D_j[]> p(new D_j[3]);
    } // 调用 ~D 3 次
}
