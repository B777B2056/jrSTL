...
TEST(testCase,unordered_multimap_iterator_ctor_test) {
    ...
    // 迭代器范围构造函数
    jr_std::unordered_multimap<int, int, int_hash> des(src.begin(), src.end());
...
TEST(testCase,unordered_multimap_copy_ctor_test) {
    ...
    jr_std::unordered_multimap<int, int, int_hash> src;
    for(size_t i = 0; i < cnt; i++){
        int t = var;
        src.insert(std::make_pair(t, i));
        src.insert(std::make_pair(t, i));
    ...
    // 拷贝构造函数
    jr_std::unordered_multimap<int, int, int_hash> des(src);
    ASSERT_EQ(src.size(), des.size());
    auto it = src.begin();
    auto dit = des.begin();
...
TEST(testCase,unordered_multimap_move_ctor_test) {
    ...
    // 移动构造函数
    jr_std::unordered_multimap<int, int, int_hash> des(std::move(src));
...
TEST(testCase,unordered_multimap_initializer_list_ctor_test) {
    ...
    jr_std::unordered_multimap<int, int, int_hash> des{{32, 0},{53423,1},{4245,2},{25,3},{234,4},{25,5},{45,6},{2,7},{235,8},{23,9},{24,10},{6,11},{47,12},{6,13},{5224,14},{2,15}};
    ASSERT_EQ(src.size(), des.size());
    auto it = src.begin();
    for(; it != src.end(); ++it) {
        bool a = false;
...
TEST(testCase, unordered_multimap_operator_copy_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    jr_std::unordered_multimap<int, int, int_hash> src{{var, 0}, {var+1, 1}, {var+2,2}};
    jr_std::unordered_multimap<int, int, int_hash> des{{32, 0},{53423,1},{4245,2},{25,3},{234,4},{25,5},{45,6},{2,7},{235,8},{23,9},{24,10},{6,11},{47,12},{6,13},{5224,14},{2,15}};
    des = src;
    ASSERT_EQ(src.size(), des.size());
    auto it = src.begin();
...
TEST(testCase, unordered_multimap_operator_move_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    jr_std::unordered_multimap<int, int, int_hash> src{{var, 0}, {var, 1}, {var, 2}};
    jr_std::unordered_multimap<int, int, int_hash> des{{32, 0},{53423,1},{4245,2},{25,3},{234,4},{25,5},{45,6},{2,7},{235,8},{23,9},{24,10},{6,11},{47,12},{6,13},{5224,14},{2,15}};
    des = std::move(src);
    ASSERT_EQ(src.empty(), true);
}
...
TEST(testCase, unordered_multimap_clear_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    jr_std::unordered_multimap<int, int, int_hash> des;
    for(size_t i = 0; i < cnt; i++){
        int t = var;
...
TEST(testCase, unordered_multimap_insert_test) {
    ...
    // 迭代器insert
    jr_std::unordered_multimap<int, int, int_hash> tmp{{1,0},{2,1},{3,2},{4,3},{5,4},{1,5},{2,6},{3,7},{4,8},{5,9}};
...
TEST(testCase, unordered_multimap_insert_hint_test) {
    ...
    jr_std::unordered_multimap<int, int, int_hash> des{{1,0},{2,1},{3,2},{4,3},{8,4},{9,5}};
    // insert hint
...
TEST(testCase, unordered_multimap_swap_test) {
    ...
    jr_std::unordered_multimap<int, int, int_hash> tmp = src;
    src.swap(des);
