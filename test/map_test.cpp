...
TEST(testCase,map_iterator_ctor_test) {
    ...
    // 迭代器范围构造函数
    jrSTL::map<int, int> des(src.begin(), src.end());
...
TEST(testCase,map_move_ctor_test) {
    ...
    // 移动构造函数
    jrSTL::map<int, int> des(std::move(src));
...
TEST(testCase,map_initializer_list_ctor_test) {
    ...
    jrSTL::map<int, int> des{{32, 0},{53423,1},{4245,2},{25,3},{234,4},{25,5},{45,6},{2,7},{235,8},{23,9},{24,10},{6,11},{47,12},{6,13},{5224,14},{2,15}};
    ASSERT_EQ(src.size(), des.size());
    auto it = src.begin();
    auto dit = des.begin();
    for(; dit != des.end(); ++dit, ++it)
...
TEST(testCase, map_operator_copy_test) {
    ...
    int var;
    get_random_size_var(MAX_SIZE, cnt, var);
    jrSTL::map<int,int> src{{var, 0}, {var, 1}, {var, 2}};
    jrSTL::map<int,int> des{{32, 0},{53423,1},{4245,2},{25,3},{234,4},{25,5},{45,6},{2,7},{235,8},{23,9},{24,10},{6,11},{47,12},{6,13},{5224,14},{2,15}};
    des = src;
    ASSERT_EQ(src.size(), des.size());
    auto it = src.begin();
...
TEST(testCase, map_insert_hint_test) {
    ...
    jrSTL::map<int,int> des{{1,0},{2,1},{3,2},{4,3},{8,4},{9,5}};
    // insert hint
